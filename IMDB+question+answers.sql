...
SELECT COUNT(m.id) AS number_of_movies, g.genre,
		DENSE_RANK() OVER(ORDER BY COUNT(m.id) DESC) AS dense_rank_genre
FROM movie m
INNER JOIN genre g
ON m.id = g.movie_id
GROUP BY g.genre;
...
SELECT COUNT(movie_id) AS single_genre_movie_count FROM genre_count_table;
...
WITH production_house_hit_movie
...
SELECT *
FROM   production_house_hit_movie
...
SELECT 
    languages, SUM(total_votes) AS tot_votes
FROM
    movie m
        INNER JOIN
...
USING (movie_id),
genre_order go
WHERE avg_rating > 8 AND g.genre IN (go.genre)
...
INNER JOIN names n 
ON rm.name_id=n.id
WHERE languages = 'Hindi' AND category LIKE '%Actress%'
GROUP BY name
HAVING COUNT(m.id) >= 3;
...
SELECT *,
        ...
        WHEN avg_rating <5.0 THEN 'Flop movie'END) AS Classification
